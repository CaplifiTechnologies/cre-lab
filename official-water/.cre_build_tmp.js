...
var UA = { "User-Agent": "Caplifi-CRE-lab/1.0 (matt@caplifi.com)" };
var fetchUsgs = (sendRequester, url) => {
...
var fetchNasa = (sendRequester, url) => {
  const response = sendRequester.sendRequest({ url, method: "GET", headers: UA }).result();
  if (!ok(response)) {
...
var usgsAgg = ConsensusAggregationByFields({
  siteName: identical,
  stageFtX100: median,
  observedAt: identical
});
var nasaAgg = ConsensusAggregationByFields({
  day: identical,
  tempCx10: median
});
var onCronTrigger = (runtime2) => {
  const http2 = new ClientCapability;
  const usgs = http2.sendRequest(runtime2, fetchUsgs, usgsAgg)(runtime2.config.usgsUrl).result();
  const lagMs = runtime2.config.nasaLagDays * 24 * 60 * 60 * 1000;
  const day = yyyymmddUtc(new Date(runtime2.now().getTime() - lagMs));
  const nasaUrl = `${runtime2.config.nasaBaseUrl}&start=${day}&end=${day}`;
  const nasa = http2.sendRequest(runtime2, fetchNasa, nasaAgg)(nasaUrl).result();
  const report2 = {
  ...
  runtime2.log(`[official-water] ${report2.usgs.site} stage ${report2.usgs.stageFt} ft · NASA T2M ${report2.nasa.t2mC} C (${report2.nasa.day})`);
  return JSON.stringify(report2);
};
